...
export default function Category({ categories, selectedCat }: CategoryProps) {
      ...
      <div className="flex flex-row items-center gap-2 lg:gap-4 overflow-x-auto no-scrollbar py-2">
        {categories.map((item) => {
          ...
          return (
            ...
            >
              <div
                className={`p-4 bg-white shadow-lg rounded-md flex items-center justify-center 
                ${selectedCat === item.category ? "border border-[#800020]" : ""}`}
              >
                <Image
                  src={item.image}
                  alt={item.category}
